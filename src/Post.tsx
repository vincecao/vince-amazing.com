...
const Post = memo(({ postId }: { postId: string }) => {
  ...
  return (
    <div className="space-y-3 py-10 px-5 font-sans font-extralight" style={{ maxWidth: "100vw" }}>
      <h1 className="text-2xl mb-5">{content.title}</h1>
      {date && <p className="italic text-right opacity-50">{date}</p>}
      <ReactMarkdown className="mx-auto prose prose-sm md:prose-base lg:prose-lg prose-zinc dark:prose-invert prose-h1:text-3xl prose-h2:text-2xl prose-h3:text-xl prose-h4:text-lg prose-h5:text-base prose-h5:font-bold" children={content.body} />
    </div>
  );
});
