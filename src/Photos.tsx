    ...
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 items-center">
      {shuffledPhotos.slice(0, 16).map(({ id, l: { url } }) => (
        <a key={id} className="md:hover:z-10 z-0" href={`//www.flickr.com/photos/saablancias/${id}/in/dateposted-public/`} target="_blank">
          <img onMouseEnter={() => setBackground(url)} onMouseLeave={() => setBackground(null)} src={url} className="w-auto md:w-[12rem] aspect-square object-cover transition ease-in-out duration-75 brightness-75 saturate-50 hover:brightness-100 hover:saturate-100 md:hover:scale-125" />
        </a>
      ))}
    </div>
  );
...
export const BlurBackground = memo(({ lowTransition }: { lowTransition?: boolean }) => {
  const background = useBackground();
  const isLoading = useBackgroundLoading();

  return (
    <div className="fixed inset-0 z-[-1] w-screen h-screen top-0 left-0 overflow-hidden">
      <AnimatePresence mode="wait">{background && <motion.div key={background} className="absolute inset-0 bg-no-repeat bg-cover blur-[80px] bg-center" style={{ backgroundImage: `url(${background})` }} initial={{ opacity: 0 }} animate={{ opacity: 0.8 }} exit={{ opacity: 0 }} transition={{ duration: lowTransition ? 0.1 : 0.6 }} />}</AnimatePresence>
